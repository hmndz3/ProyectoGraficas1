...
fn draw_tarot_card(x: f32, y: f32, w: f32, h: f32, idx: usize, sel: bool, t: f32) {
    ...
    if sel {
        ...
        draw_rectangle(
            ...
            w + 16.0,
            h + 16.0,
            Color::new(accent.r, accent.g, accent.b, 0.25 + 0.2 * g),
        );
    }
    ...
    match idx {
        ...
        1 => {
            ...
            draw_circle(
                ...
                cy + 34.0,
                16.0,
                Color::new(accent.r, accent.g, accent.b, 0.3),
            ); // halo
        }
        _ => {
            // El Ermitaño: figura con farol
            draw_triangle(
                vec2(cx - 22.0, cy + 50.0),
                vec2(cx, cy - 30.0),
                vec2(cx + 22.0, cy + 50.0),
...
pub fn draw_menu(sel: &mut usize, t: f32) -> Option<usize> {
    ...
    center_text(
        "un raycaster de tarot",
        158.0,
        26,
        Color::new(0.7, 0.65, 0.8, 1.0),
    );
...
pub fn draw_hud(
    ...
    draw_circle(
        ...
        hy + 18.0,
        14.0,
        Color::new(accent.r, accent.g, accent.b, g),
    );
    draw_circle(
        hx,
        hy + 18.0,
        24.0,
        Color::new(accent.r, accent.g, accent.b, 0.30 * g),
    );
    if flash > 0.0 {
        draw_circle(
            hx,
            hy + 12.0,
            34.0 + flash * 40.0,
            Color::new(1.0, 0.95, 0.6, flash * 0.9),
        );
        draw_circle(
            sw / 2.0,
            sh / 2.0,
            90.0 * flash,
            Color::new(1.0, 0.9, 0.5, flash * 0.25),
        );
    }
    ...
    draw_rectangle_lines(
        ...
        82.0,
        2.0,
        Color::new(0.9, 0.8, 0.5, 0.8),
    );
    draw_text(
        format!("SELLOS  {}/{}", cards, total_cards),
        26.0,
        sh - 72.0,
        22.0,
    ...
    draw_text(
        format!("{} - {}", CARD_NUMERALS[level_idx], CARD_NAMES[level_idx]),
        26.0,
        40.0,
        24.0,
        CARD_COLORS[level_idx],
    );
    draw_text(
        format!("Espiritus restantes: {}", spirits_left),
        26.0,
        62.0,
        18.0,
...
pub fn draw_success(
    ...
    center_text(
        &format!("Tiempo: {:.1} s", time_s),
        600.0,
        26,
        Color::new(0.85, 0.85, 0.95, 1.0),
    );
    center_text(
        &format!("Espiritus disipados: {}", spirits),
        632.0,
        26,
        Color::new(0.7, 0.85, 1.0, 1.0),
    );
    let hint = if last {
        "ESPACIO para ver tu destino"
    } else {
        "ESPACIO para el siguiente arcano  |  ESC menu"
    };
